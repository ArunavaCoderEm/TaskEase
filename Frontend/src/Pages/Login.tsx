...
export default function signup() {
    ...
    const postdatal = async (em:string, pas:string) => {
      ...
      await axios.post("/login",data).then((res:any) => {
  ...
  return (
    <>
        ...
        <form className='m-2 px-3' onSubmit={handlesubmit}>
            ...
            <div className='flex'>
            <input type= {(pass) ? 'text' : 'password'} className='w-full placeholder-green-600 text-green-600 border-2 border-solid focus:border-green-600 focus:ring-0 focus:outline-none h-10 text-center rounded-md' placeholder='Enter Password' onChange={(e) => setpassw(e.target.value)} required/>
            {(!pass) ? 
